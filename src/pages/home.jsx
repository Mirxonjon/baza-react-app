...
function Home() {
  ...
  const dateFormat = `${datenow.getFullYear()}-${
  ...
  const handleEditOpenBtn = (id, active) => {
    if (active) {
      ...
      document.querySelector('.edit_License_type').value =
        findUser.License_type;
    } else {
    ...
    <div>
      <Form className="addform editForm" onSubmit={handlEditBtn}>
        <button
          ...
          onClick={handleEditOpenBtn.bind(null, null, false)}
          type="button"
        ...
        <FormGroup className="divForm">
          ...
          <Input
            className="input edit_full_name"
            id="name"
            name="name"
      ...
      <Table bordered>
        <thead>
          <tr>
            ...
            <th>Tugash kuni</th>
            <th>Turi</th>
            <th style={{ width: '100px' }}>O'zgartirish</th>
          ...
          {users.length
            ? users.map((e, i) => (
                <tr
                  key={i}
                  style={
                    convertorDateToDay(e.finish_day) -
                      convertorDateToDay(dateFormat) <
                    0
                      ? { backgroundColor: 'red' }
                      : convertorDateToDay(e.finish_day) -
                          convertorDateToDay(dateFormat) <
                ...
                >
                  <th scope="row">{i}</th>
                  <td>{e.full_name}</td>
                  <td>{e.passport_seria}</td>
                  <td>{e.date_birth}</td>
                  <td>{e.region}</td>
                  <td>{e.get_date}</td>
                  <td>{e.finish_day}</td>
                  <th>{e.License_type}</th>
